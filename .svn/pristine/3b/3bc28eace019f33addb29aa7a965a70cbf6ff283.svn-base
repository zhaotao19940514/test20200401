...
jQuery(function() {
    ...
    // 文件接收服务端。
    server: rootUrl+'app/customerservice/card/cancelrefund/handleFile',
			...
			$.ajax({
				...
				success : function(response) {
					if(response==null||response==''||response=='undefined'){
						...
						$.Taiji.hideLoading();
						var msg = "<tr><td style='width:60px'>卡号</td><td style='width:60px'>错误信息</td></tr>";
						for(var i=0;i<response.length;i++){
							msg+="<tr><td style='width:60px'>"+response[i].cardId+"</td><td style='width:120px'>"+response[i].errorMsg+"</td></tr>";
						}
						$("#msgTab").html(msg);
						$("#showModel").click();
						$("#closeBtn").click();
