...
$(function(){
	...
	var provider=0;
	$("#my-table").on('click','#hangBtnOff',function(){
		$.Taiji.showWarn("卡状态为正常或挂失才能挂起");
	});
	$("#my-table").on('click','#hangBtn',function(){
		var cardId = $(this).attr("param");
		$.Taiji.defConfirm("你确定要对该卡进行挂起？").done(function(){
			doHang(cardId,provider);	
		});
	});
...
function doHang(cardId,provider){
	$.Taiji.showLoading($.Taiji.Messages.MSG_OPERATE);
	  var data = {};
	  data.cardId = cardId;
	  data.provider = provider;
	  $.ajax({
          url : rootUrl+"app/customerservice/card/hang/hangCard",
          type : "POST",
          dataType : "json",
			    ...
			    <div class="col-md-12">
                    ...
                    <div id="myManage" class="panel panel-inverse">
                        <div class="panel-heading">
                            ...
                            <h4 class="panel-title">卡片挂起</h4>
                        </div>
                        <div class="panel-body">
                        	<form:form modelAttribute="queryModel" cssClass="taiji_search_form form-inline m-t-5 " id="listForm" name="listForm" action="${rootUrl }app/customerservice/card/hang/manage" method="POST">
