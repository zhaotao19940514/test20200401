...
CardReader.fn = CardReader.prototype = {
	...
	init : function() {
		//chrome
		this.objStr = '<object id="vfjCardReader" type="application/x-itst-activex" width="0" height="0" clsid="{db83c620-81dc-11e1-b0c4-0800200c9a66}" progid="${rootUrl}cardReaderOcx/CardReaderOcx.ocx"></object>';
		//ie
//		this.objStr = '<object id="vfjCardReader"  classid="clsid:{db83c620-81dc-11e1-b0c4-0800200c9a66}" codebase="${rootUrl}cardReaderOcx/CardReaderOcx.ocx" style="display:none;" ></object>';
		if($("#vfjCardReader").length > 0){
			 $("#vfjCardReader").remove();
		}
		$("body").prepend(this.objStr);
		this.reader = $("#vfjCardReader").get(0);
		this.setInfo();
	...
	setInfo : function(){
		...
		var retType = 4;
		var comBaud = 38400;
		var bbHost = '127.0.0.1';// 不暴露真实IP
		var bbPort = 12345;
		var result = this.reader.SetInfo(retType, comPort, comBaud, bbHost,	bbPort);
		return result;
	},
	getBalance : function() {
...
//		console.log(nameValuePair[1]);
	...
	getCardPlateColor : function() {
	    this.openCard();
		var cardVersion = getCardVersion(this);
		//高4位
