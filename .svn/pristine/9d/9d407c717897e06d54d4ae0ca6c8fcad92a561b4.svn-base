...
<script type="text/javascript">
	$(function() {
		...
		$("#submit").click(function() {
		  ...
		  var accountName = '${editModel.account.name}';
	        var accountId= '${editModel.accountId}';
	        $("#editForm .chosen-single span").text(accountName);
	        $("#accountId").append('<option selected="selected" value="'+accountId+'">'+accountName+'</option>');
	});
</script>
</head>
<body>
	<div class="modal-header">
		...
		<h4 class="modal-title">机构管理--修改</h4>
	</div>
	<div class="modal-body">
		<form:form modelAttribute="editModel" id="editForm" name="editForm"
			cssClass="form-horizontal"
			action="${rootUrl }app/administration/agency/edit" method="post">
			...
			<table class="table table-bordered">
				...
				<th>资金渠道:</th>
					<td>
						<form:select path="accountId" cssClass="taiji_autocomplete form-control" data-url="${rootUrl }app/administration/agency/queryByNameForModal" data-placeholder="请选择资金渠道" >
                            <option></option>
                        </form:select>
                    </td>
                </tr>
			...
			<table class="table table-bordered">
				...
				<tr>
					<th><label class="control-label">流水目录配置</label></th>
					<td colspan="3"><form:input path="fileDir"
							cssClass="form-control" maxlength="256" placeholder="" readonly="readOnly"/></td>
				</tr>
			</table>
		</form:form>
	</div>
