...
$(function(){
	...
	obuStatus=5;
	$("#my-table").on('click','#oCancelBtnOff',function(){
		var param1=$(this).attr("param1");
		if(param1=='0'||param1=='11'){
			$.Taiji.showWarn("请先离线安装");
		}else if(param1=='4'||param1=='5'){
			$.Taiji.showWarn("该OBU已注销");
		}else{
			$.Taiji.showWarn("该OBU不能办理注销业务，正常、挂起或挂失状态才能办理");
		}
	});
	$('#my-table').on('click','#oCanBtn',function(){
		var obuId = $(this).attr("param");
		$.Taiji.defConfirm("你确定要对此电子标签执行注销？").done(function(){
			docancel(obuId,obuStatus);
		});
	});
...
function docancel(obuId,obuStatus){
	$.Taiji.showLoading($.Taiji.Messages.MSG_OPERATE);
	var data={};
	data.obuId = obuId;
	data.obuStatus = obuStatus;
	$.ajax({
		url:rootUrl+"app/customerservice/obu/cancel/doOBUCancel",
			    ...
			    <div class="col-md-12">
                    ...
                    <div id="myManage" class="panel panel-inverse">
                        <div class="panel-heading">
                            ...
                            <h4 class="panel-title">OBU注销</h4>
                        </div>
                        <div class="panel-body">
                        	<form:form modelAttribute="queryModel" cssClass="taiji_search_form form-inline m-t-5 " id="listForm" name="listForm" action="${rootUrl }app/customerservice/obu/cancel/manage" method="post">
								...
								<button class="taiji_search_submit btn btn-md btn-success m-r-5" type="button" id="searchBtn" ><i class="fa fa-search  m-r-10 "></i>查询</button>
