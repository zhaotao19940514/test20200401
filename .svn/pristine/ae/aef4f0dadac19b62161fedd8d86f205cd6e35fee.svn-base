	...
	<td>
		<c:choose>
			<c:when test="${vo.status eq '1'||vo.status eq '6' }">
				<a param="${vo.obuId}" id="oHangBtn" href="javaScript:void(0)"  class="taiji_acl">挂起</a>
			</c:when>
			<c:otherwise>
				<a id="oHangBtnOff" href="javaScript:void(0)"  class="taiji_acl" style="opacity: 0.2">挂起</a>
			</c:otherwise>
		</c:choose>
