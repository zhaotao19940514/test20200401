		...
		$(function(){
			...
			$("#submit").click(function(){
				...
				if(status==4 || status==5){
					$.Taiji.showWarn("此卡已被注销，无法进行卡解挂!");
					return;
				}
				if(status==9){
					$.Taiji.showWarn("此卡已被预注销，无法进行卡解挂!");
					return;
				}
				if(isNaN(card)){
				...
				if(!card==cardId){
				  ...
				  if(confirm("确定要给"+cardId+"这张卡修改信息并解挂？\n\n请确认！")){
					 	var data = {};
					 	data.cardId=cardId;
		   ...
		   function applyCardOrderConfirm(dataObj){
...
<form:form modelAttribute="pageModel" id="myForm" name="myForm" cssClass="form-horizontal" action="${rootUrl }app/customerservice/card/cardInformationChange/edit" method="post">
		<div class="form-group">
			...
			<table border="1">
				<tr>
					<td class="titile">ETC号：</td><td class="details"><form:input cssStyle="width:180px" path="cardId" id="ETCcardId" readonly="true" class="dataChange form-control" maxlength="9" /></td>
				</tr>
				<tr>
				...
				<tr>
					<td class="titile">证件号码：</td><td class="details"><form:input cssStyle="width:180px" path="customerIdNum" id="customerIdNum"  readonly="true" class="dataChange form-control" maxlength="9" /></td>
				</tr>
			    <tr>
