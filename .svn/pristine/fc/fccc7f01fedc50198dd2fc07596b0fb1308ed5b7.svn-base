...
$(function(){
	...
	$("#my-table").on('click','#oHangBtnOff',function(){
		$.Taiji.showWarn("OBU状态为正常或挂失才能执行挂起业务");
	});
	$('#my-table').on('click','#oHangBtn',function(){
		var obuId = $(this).attr("param");
		$.Taiji.defConfirm("你确定要对此电子标签执行挂起？").done(function(){
			doHangObu(obuId,provider);
		});
	})
...
function doHangObu(obuId,provider){
	$.Taiji.showLoading($.Taiji.Messages.MSG_OPERATE);
	var data={};
	data.obuId = obuId;
	data.provider = provider;
	$.ajax({
		url:rootUrl+"app/customerservice/obu/hang/doHangObu",
			    ...
			    <div class="col-md-12">
                    ...
                    <div id="myManage" class="panel panel-inverse">
                        <div class="panel-heading">
                            ...
                            <h4 class="panel-title">OBU挂起</h4>
                        </div>
                        <div class="panel-body">
                        	<form:form modelAttribute="queryModel" cssClass="taiji_search_form form-inline m-t-5 " id="listForm" name="listForm" action="${rootUrl }app/customerservice/obu/hang/manage" method="post">
