...
// 				debugger;
		        ...
		        if(packageNum !=null && packageNum != ""){
		        	$.post(url+"app/apply/emergency/equipmentissue/moneyPlayType/"+packageNum,function(data,status){
			            if(status=="success"){
			            	if(data.success){
...
// 				debugger;
				...
				if(packageNum == null || packageNum == "" || issueType == null){
					...
					/* if(vehicleType == 11 || vehicleType == 12 || vehicleType == 13 || vehicleType == 14 || vehicleType == 15){
						if(issueType != 1){
...
// 					if(issueType == 3) type = 0;
			...
			function vehiclePlateOnlyCheck(type,vehicleId) {
				...
				$.ajax({
					url:url+"app/apply/emergency/equipmentissue/vehiclePlateOnlyCheck",
			...
			function saveCarIssuePackage(packageNum,vehicleId) {
				...
				$.ajax({
					url:url+"app/apply/emergency/equipmentissue/saveCarIssuePackage",
					data:JSON.stringify(data),
					type:"POST",
					contentType : 'application/json',
			        dataType : 'json',
...
// 				debugger;
			...
			function payCarIssuePackage(responsePackage) {
				...
				$.ajax({
					url:url+"app/apply/emergency/equipmentissue/payCarIssuePackage",
					data:JSON.stringify(data),
					type:"POST",
					contentType : 'application/json',
			        dataType : 'json',
				...
				<tr>
					...
					<td class="details">
						<select name="packageNum" id="packageNum" class="addDisabled form-control m-r-5" data-style="btn-white" data-width="165px">
							...
							<option value="3">0元送卡送OBU</option>
							<option value="4">0元送卡</option>
							<option value="5">0元送OBU</option>
