...
public class Obu4XSectionManageImpl implements Obu4XSectionManage {
	...
	public void add(Obu4XSectionRequest request, String staffId) throws ManagerException {
		// 参数校验
		request.paramCheck();
		Long startIdCheck = Long.valueOf(request.getStartId());
		Long endIdCheck = Long.valueOf(request.getEndId());
		...
		// 写入日志
		Operation4xLog log = getLog(staffId, request.getStartId(), request.getEndId(), DeviceVersion.getDeviceVersionFromCode(request.getVersion()),
				UUID.randomUUID().toString(), LocalDateTime.now().toString().substring(0, 19), Operation4xType.ADD_OBU);
	...
	public void batchAdd(Obu4XSectionRequest request, String staffId) throws ManagerException {
		...
		for (String section : request.getBatchInfo()) {
			...
			Long startId = Long.valueOf(splitResult[0].toString());
			Long endId = Long.valueOf(splitResult[1].toString());
			if (endId < startId) {
		...
		for (String operationDeatil : obuSection) {
			...
			log.setDeviceVersion(DeviceVersion.getDeviceVersionFromCode(request.getVersion()));
			log.setBatchNo(uuid);
	...
	private void batchAddParamCheck(Obu4XSectionRequest request) throws ManagerException {
		if (request.getBatchInfo().size() == 0)
			throw new ManagerException("请填写OBU批量信息");
		if (request.getVersion() == null)
			throw new ManagerException("请填写OBU版本");
		if (request.getVersion() != 2)
			throw new ManagerException("OBU版本选择错误");
	}
}
