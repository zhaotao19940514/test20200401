...
public class CancelManagerImpl extends AbstractDsiCommManager implements CancelManager {
	...
	public AppCardStatusChangeResponse doPreCancel(@Valid PreCancelRequest queryModel, User user) throws ManagerException {
		...
		if (queryModel.getApplyStep() == 1) {
			...
			if(queryModel.getCardType()==1) {
				if(checkCancelReleasePlate(user.getStaff().getServiceHall().getAgencyId())) {
...
//			else if(!queryModel.isSupplyOrCancel()){
//				toEntityOprt(queryModel, user);
//				cardCOSRes = preCancelWithCOS(queryModel, user);
//			}else {
...
//			}
	...
	public CardCancelResponse doCancel(@Valid CancelRequest queryModel, User user) throws Exception {
		...
		if (cardInfoQueryResponse.getStatus() == 1) {
			...
			if (status == 9) {
				if(cardInfoQueryResponse.getCardType()/100==2) {
					CancelledCardDetail detail = cancelDetailRepo.findByCardId(queryModel.getCardId());
					if(null==detail) {
						supplyCancelDetail(queryModel, user);
					}
				}
	...
	private void toEntityOprt(PreCancelRequest queryModel,User user) {
		CustomerInfo cusInfo = customerInfoRepo.findByCustomerId(cardInfoRepo.findByCardId(queryModel.getCardId()).getCustomerId());
	...
	private CardCancelWithCOSResponse preCancelWithCOSV2(PreCancelRequest queryModel, User user) {
		...
		cardCancelWithCOSV2Request.setOperateType(queryModel.getRefundType());
		if(queryModel.isSupplyOrCancel())
			cardCancelWithCOSV2Request.setBankType(queryModel.getBankType());
		cardCancelWithCOSV2Request.setCardBalance(0L);
		if(queryModel.getProvider()==1) {
			cardCancelWithCOSV2Request.setCardBalance(queryModel.getCardBalance());
		}
		if(null!=queryModel.getCusType()&&queryModel.getCusType()==2) {
...
//		return getFilePathlWithOutRoot(destDirPath);	
	...
	private void supplyCancelDetail(CancelRequest req,User user) {
		CardInfo cardInfo = cardInfoRepo.findByCardId(req.getCardId());
		CancelledCardDetail detail = new CancelledCardDetail();
		detail.setCancellationTime(new SimpleDateFormat("yyyyMMdd").format(new Date()));
		detail.setCardId(req.getCardId());
		detail.setInvoiceAmount((long) 0);
		detail.setCreateTime(Calendar.getInstance());
		detail.setStatus(0);
		detail.setCardType(req.getCardType()/100);
		detail.setAgencyId(user.getStaff().getServiceHall().getAgencyId());
		detail.setChannelId(user.getStaff().getServiceHallId());
		detail.setStaffId(user.getStaffId());
		detail.setUserIdNum(cardInfo.getCustomer().getCustomerIdNum());
		detail.setVehicleId(cardInfo.getVehicleId());
		detail.setReplaceStatus(0);
		cancelDetailRepo.save(detail);
	}
}
