...
public class CardRefundConfirmRequest extends JpaDateTimePageableDataRequest<CancelledCardDetail> {
			...
			public void validate() {
				...
				if(null==cardId&&null==afterDate&&null==beforeDate&&null==refundType) {
					mve.addViolation("cardId", "请填写卡号");
				}
			...
			public HqlBuilder toSelectHql() {
				...
				HqlBuilder hql = new HqlBuilder("from " + "CancelledCardDetail" +
						"  where 1=1 and cardType =2");
				hql.append(" and cardId=:cardId", cardId);
				if(null!=afterDate) {
					Calendar beforDate = getCalenderDate(beforeDate);
					Calendar afteDate = getCalenderDate(afterDate);
					hql.append(" and createTime between :beforDate",beforDate);
					hql.append(" and :afteDate",afteDate);
				}
