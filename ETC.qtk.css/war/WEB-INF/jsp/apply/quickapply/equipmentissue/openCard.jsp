...
// 				$.ajax({
// 					url:url+"app/apply/quickapply/equipmentissue/carIssuePackageCheck",
...
// 			};
			...
			function cardInventoryCheck(cardReader) {
				...
				if(cardId == null || cardId == "" || isNaN(cardId)){
					...
					data.cardId = cardId;
					$.ajax({
						url:url+"app/apply/quickapply/equipmentissue/inventoryVerify",
						data:JSON.stringify(data),
						type:"POST",
						contentType : 'application/json',
				        dataType : 'json',
	...
	// 			debugger;
				...
				var pkgId = $("#pkgId").val();
				var url = '${rootUrl}'+"app/apply/quickapply/equipmentissue/cardApplyAndConfirm";
				...
				cardReader.autoExecuteApdu(url,data,method,terminateConditions,function(){
					...
					if(vehicleType == 1 || vehicleType == 2 || vehicleType == 3 || vehicleType == 4){
						var prompt = "卡发行成功！客车请点击确定进行卡签绑定！若绑定出错，请去卡签绑定页面绑定！";
				...
				<table border="1">
					<tr>
						<td class="titile"><span class="essential">*</span>发行类型：</td><td class="details" >
														...
														<span><input type="radio" name="cardTypeChoose" value="23">记账卡</span>
														</td>
