...
//	     }
//		String dateDayStr = DateTimeFormatter.ofPattern("yyyyMMdd").format(LocalDate.now().plusDays(-1));
//		LocalDate time = LocalDate.of(2019, 6, 1);
...
//		}
//		SimpleDateFormat format3 = new SimpleDateFormat("yyyyMMddHHmmSSS");
//		Date fileDate = new Date();
//        String filetimeStamp = format3.format(fileDate);
//        System.out.println(filetimeStamp);
		/*
		 * Date date = new Date(); Calendar calendar = Calendar.getInstance();
		 * calendar.setTime(date); calendar.add(Calendar.HOUR_OF_DAY, -10);
		 * System.out.println(LocalDateTime.now().minusHours(hours));
		 */
		///LocalDate startDate = LocalDate.parse("2019-01",DateTimeFormatter.ofPattern("yyyy-MM")).plusMonths(1);
	...
	public void uniqueVehiclePlateCheck() throws ManagerException {
		...
		commset(req);
		req.setVehiclePlate("贵A3DF59");
		req.setVehicleColor(0);
		req.setVehicleType(1);
		req.setReleaseType(2);
	...
	@Test
	public void cardRefundConfirmTest() {
		for (int i = 0; i < 6; i++) {
			List<String> cardIdsByDate = accountRefundDetailRepo.cardIdsByCancelDate();
			// List<String> cardIdsByDate =
			// Lists.newArrayList(accountRefundDetailRepo.findByCardId("52011411220300454407").getCardId());
			logger.info("size:" + cardIdsByDate.size());
			for (String cardId : cardIdsByDate) {
				System.out.println(cardId);
				CardAccountRefundRequest req = new CardAccountRefundRequest();
				req.setCardId(cardId);
				try {
					User user = new User();
					user.setStaffId("zt001");
					cardRefundConfirmManager.queryPage(req, user);
