...
$(function(){
	...
	$("#my-table").on('click','#compleRefund',function(){
		...
		var refundType = $(this).attr("refundType");
		if(refundType=='GLYQR'){
			$.Taiji.defConfirm("确定完成该笔退款吗？").done(function(){
	...
	$("#my-table").on('click','#revereRefund',function(){
		var refundCardId = $(this).attr("refundCardId");
		console.log("refundCardId:"+refundCardId);
			$.Taiji.defConfirm("确定冲正该笔退款吗？冲正之后将重新计算退款金额").done(function(){
				confirmrevereRefund(refundCardId);
			});
	});
	$("#my-table").on('click','#againCheck',function(){
		var refundCardId = $(this).attr("refundCardId");
			$.Taiji.defConfirm("确定重新核定退款金额？").done(function(){
				againCheck(refundCardId);
			});
		});
...
function confirmrevereRefund(cardId){
	$.Taiji.showLoading($.Taiji.Messages.MSG_OPERATE);
	var data = {};
		data.refundCardId = cardId;
		data.cardId = cardId;
	$.ajax({
	      url : rootUrl+"app/customerservice/finance/cardrefundcost/revereRefund",
	      type : "POST",
	      dataType : "json",
...
function againCheck(cardId){
	$.Taiji.showLoading($.Taiji.Messages.MSG_OPERATE);
	var data = {};
		data.refundCardId = cardId;
	$.ajax({
	      url : rootUrl+"app/customerservice/finance/cardrefundconfirm/againCheck",
	      type : "POST",
	      dataType : "json",
	      data:JSON.stringify(data),
	      contentType: "application/json",
			    ...
			    <div class="col-md-12">
                    ...
                    <div id="myManage" class="panel panel-inverse">
                        <div class="panel-heading">
                            ...
                            <h4 class="panel-title">注销退费</h4>
                        </div>
                        <div class="panel-body">
                        	<form:form modelAttribute="queryModel" cssClass="taiji_search_form form-inline m-t-5 " id="listForm" name="listForm" action="${rootUrl }app/customerservice/finance/cardrefundcost/manage" method="POST">
