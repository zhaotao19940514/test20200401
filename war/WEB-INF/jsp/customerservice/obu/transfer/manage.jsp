...
$(function(){
	...
	$("#my-table").on('click','#oTransferBtnOff',function(){
		$.Taiji.showWarn("该车辆已绑定OBU!请先注销该车辆正在使用的OBU");
	});
	/* $("#my-table").on('click','#oTransferBtn',function(){
		var obuId = $(this).attr("param");
		$.Taiji.defConfirm("你确定要对该OBU进行过户？").done(function(){
			doLossOBU(obuId);	
		});
	}); */
...
/* function doLossOBU(obuId){
	$.Taiji.showLoading($.Taiji.Messages.MSG_OPERATE);
	  var data = {};
	  data.obuId = obuId;
	  $.ajax({
          url : rootUrl+"app/customerservice/obu/loss/lossOBU",
          type : "POST",
          dataType : "json",
			    ...
			    <div class="col-md-12">
                    ...
                    <div id="myManage" class="panel panel-inverse">
                        <div class="panel-heading">
                            ...
                            <h4 class="panel-title">OBU过户</h4>
                        </div>
                        <div class="panel-body">
                        	<form:form modelAttribute="queryModel" cssClass="taiji_search_form form-inline m-t-5 " id="listForm" name="listForm" action="${rootUrl }app/customerservice/obu/transfer/manage" method="POST">
								<div class="form-group">
									...
									<form:select path="vehiclePlateColor"  cssClass="form-control m-r-5" data-style="btn-white" data-width="100px">
										...
										<form:options items="${vehiclePlateColorTypes}" itemLabel="value" itemValue="code"/>
									</form:select> --%>
								</div>
									...
									<%-- <form:input path="ownerIdNum"  maxlength="100"  cssClass="form-control"  placeholder="证件号码" />
									<form:select path="ownerIdType"  cssClass="form-control m-r-5" data-style="btn-white" data-width="150px">
										<form:option value="">--请选择证件类型--</form:option>
